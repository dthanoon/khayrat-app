...
class WidgetSyncModule: NSObject {
  ...
  func syncData(_ data: NSDictionary) {
    ...
    defaults.set(data["refreshToken"]    as? String ?? "", forKey: "refresh_token")
  ...
  @objc
  func syncAuthToken(_ userId: String, accessToken: String, refreshToken: String, supabaseUrl: String, supabaseAnonKey: String) {
    guard let defaults = UserDefaults(suiteName: "group.com.khayrat.app") else { return }
